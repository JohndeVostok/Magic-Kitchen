...
def new_solution(request):
    ...
    if not 'level_id' in content:
        ret['error'] = 1027 #'level id can\'t be empty'
        return json_response(ret)
    ...
    if not 'solution_info' in content:
        ret['error'] = 1023 #'solution info can't be empty'
        return json_response(ret)
    _solution_info = content['solution_info']

    if not 'score' in content:
        ret['error'] = 1024 #'score can't be empty'
        return json_response(ret)
    ...
    try:
        ...
        print e
        ret['error'] = 1025 #'the input score needs to be an Integer'
        return json_response(ret)

    if _score > 4 or _score < 0:
        ret['error'] = 1026 #'the input score needs to be in range[0,4]'
        return json_response(ret)
